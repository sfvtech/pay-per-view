...
public class SubtitleView extends android.support.v7.widget.AppCompatTextView implements Runnable {
    ...
    public void setSubSourceFromFile(String uriString, String mime) {
        if (mime.equals(MediaPlayer.MEDIA_MIMETYPE_TEXT_SUBRIP))
            track = getSubtitleFileFromPath(uriString);
        else
            throw new UnsupportedOperationException("Parser only built for SRT subs");
    }

    private TreeMap<Long, Line> getSubtitleFileFromPath(String uriString) {
        InputStream inputStream = null;
        try {
            File initialFile = new File(Uri.parse(uriString).getPath());
